...
export const updateLectureProgress = async(req, res) => {
    try {
        ...
        if(lectureIndex !== -1){
            // If lecture already exists, update it's status
...
export const markAsCompleted = async(req, res) => {
    try {
        ...
        await courseProgress.save();
        return res.status(200).json({message: "Coures marked as completed."});
    } catch (error) {
        console.log(error);
    }
};
...
export const markAsInCompleted = async(req, res) => {
    try {
        ...
        await courseProgress.save();
        return res.status(200).json({message: "Coures marked as incompleted."});
    } catch (error) {
        console.log(error);
    }
};